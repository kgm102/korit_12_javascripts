...
1. LocalStroratge
  - 저장된 데이터를 삭제하기 전까지 영구히 보존됨. 저장된는 모든 데이터는 JS 객체처럼 key - value pair로 이루어져있습니다. 단, 저장되는 모든 데이터 값은 string으로만 저장가능합니다. 그래서 이 저장소에 number, object, boolean, array 등과 같은 string이 아닌 데이터를 저장할 때는 JSON. stringfy를 통해 문자열로 변환하여 저장해야 합니다. 그리고 localStorage의 데이터를 JS로 읽고 싶다면 JSON.parse()를 사용하여 객체로 변환해줘야 연산이 가능합니다.
...
console.log(users);
console.log(SON.stringify(users));
...
12:00 부터 html - css - js를 합친 예시 하나 작성할 예정.
todolist

ch07_todolist
todolist.html / todolist.css /todolist.js

todolist 작성한 부분을 github에 올릴건데 중요한 부분을 ai 사용하고 나서 어떻게 붙여넣을지 입니다.

- todolist.html을 기준으로 보냈기 때문에 .css와 .js가 기존 버전으로 되어있어서 html만 BEM 버전으로 바꾼다고 하더라도 실제로는 CSS가 적용되지 않습니다.

- 특정 방법을 사용했을 때 여러분이 자겅한거 다 날리고 덮어쓸 수도 있겠지만 남겨놓고 CSS 버전만 다양하게 갈아끼우고 싶다면 남겨두는 방법도 필요합니다.

- 레퍼런스를 아마 AI로 만들 확률이 높기 때문에 기존 문서에서 적용하기 위해서는 link 태그 사용후에 href를 잘 적용시켜야합니다.
