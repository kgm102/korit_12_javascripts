...
2. todolist를 shoppinglist로 개조할거라고 했습니다. ch08_shoppinglist -> 01_shoppinglist.html / css / js

- input 창에 가격을 추가하겠습니다.

- js 부분을 수정해야 될 것 같습니다. renderTodo() 부분에 일단 html 태그가 추가되어야 합니다. span / div

- 지시 사항
html 태그에 있는 todolist 기준의 BEM을 전부 shoppinglist 기준 BEM으로 수정할 것 -> AI사용하셔도 무방합니다.
...
- localStorage에 todo / shopping item 을 추가하고 읽어오고 삭제까지 해봤습니다.

이번 수업 때는 jsonplaceholder를 활용하여 CRUD를 구현해볼겁니다. -> Fetch API를 학습할겁니다.

https://jsonplaceholder.typicode.com을 기준으로
1. /posts
2. /comments
3. /albums
4. /photos
...
<body bgcolor="5f5f5f">
  ...
  <script>
    ...
    fetch("https://jsonplaceholder.typicode.com/posts",{
      method: "POST", // http request 요쳥 방문
      body: JSON.stringify({
        title: 'foo',
        body: 'bar',
        userId: 1,
    ...
    // 근대 삭제 됐는지 확인할 수가 없기 때문에 Read를 새로 해야 합니다.
    fetch("https://jsonplaceholder.typicode.com/posts/1")
    .then(response => response.json())
    .then((json) => console.log(json.body));
    // 그러니까 삭제 후에는 todolist 기준으로 redndorTodo를 새로 해야겠네요.
...
- 원래 웹은 요청(request)에 대한 반응(response)이 _순차적으로 이루어지고_ , 먼저 실행된 코드가 완료되어야 다음 코드를 실행하느 순서를 가지고 있었는데, 개선 이후에 요청을 보낸 후 요청에 댛나 응답이 돌아오는 것과 관계 없이 다음 코드를 실행시킬 수 있게 되었습니다.
...
myFunction();
// 이상의 예시는 Get 요청을 해서 서버로부터 응답이 오면 콘솔에 찍는 함수입니ㅏㄷ.
...
- 웹 페이지를 이용하면 웹 화면과 사용자 사이에서 많은 상호작용이 있습니다.
버튼 클릭, 마우스 스크롤, 검색 키워드를 입력하는 등의 행동이 해당됩니다.
사용자가 웹 페이지를 이용하면서 행하는 행위를 event라고 합니다.

1. click 이벤트 : onclick
사용자가 웹 페이지 내에서 마우스 왼쪽 버튼 클릭을 할 때 발생하는 이벤트. 웹 화면서에서 일어나는 모든 클릭이벤트를 감시할 필요는 없지만 '검색', '저장' 처럼 사용자가 클릭을 하고 나면 연관된 기능을 처리해야 하는 클릭 이벤트에 대해서는 JS에서 함수를 정의하여 확인할 필요가 있습니다.(addTodo()를 예로 들 수 있습니다.)
...
select는 여러 개의 옵션에서 하나 이사으이 옵션을 선택할 때 사용하는 DOM 요소로, 사용자가 옵션을 바꿀 때 마다 웹에서는 change 이벤트가 발생되고, JS에서 이 이벤트를 감시하면 특정함수를 실행하는 방식을 택합니다. onchange 속성을 활용합니다.
...
- key 이벤트는 사용자가 입력창에서 키보드를 입력할 때 발생합니다. 사용자가 키를 입력하면 keydown, keypress, keyup순으로 이벤트가 발생합니다. 예제를 통해 확인하겠스빈다.
...
<body bgcolor="5f5f5f">
  ...
  <script>
    function myFunction(e) {
      ...
      console.log(e.target.value); // 이벤트 입력 시점에 입력된 값이 어떻게 변하는지 출력 -> e.target.value는 input 태그에서 그냥필수적이라고 보셔도 됩니다.
    }
  </script>
</body>
</html>
